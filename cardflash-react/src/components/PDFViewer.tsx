  ...
  <iframe
    ...
    onLoadCapture={async (e) => {
      const w = e.currentTarget.contentWindow as
        | (Window & { PDFViewerApplication?: PDFViewerApplication; PDFViewerApplicationOptions: unknown })
        | null;
      if (w) {
        if (w) {
    ...
    ref={pdfIFrame}
    src={`/pdfjs/viewer/viewer.html?file=${encodeURI(props.file)}&page=${props.defaultPage ?? 1}`}
  />)}</>
}